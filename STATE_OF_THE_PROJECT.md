...
| LOT D (F4 online learning) | **PSI (Population Stability Index)** sur les features clés : `core/drift_psi.py` — `compute_psi` (bucketing par percentiles de la référence, seuils standards 0.10/0.25), `DriftMonitor` (fenêtres 400/150 barres, features RÉELLES quasi-iid : returns_1, \|returns\|, momentum_10, log(volume)), `run_drift_check` (tick périodique 15 min dans la boucle live, jamais bloquant). **Oubli du bandit ACCÉLÉRÉ au drift** : `strategies/engine.py` — decay dynamique `set_bandit_decay()` (nominal 0.98 → 0.92 en sévère, borné [0.85, 0.995], jamais d'oubli total) + paramètre `decay` dans `update_bandit_feedback`. **Exposé dans télémetry** (`drift_psi` : psi par feature, max_psi, statut, decay appliqué). **Leçon de calibrage documentée** : le PSI par percentiles est un test de RANG (invariant par transformation monotone — le z-score ne sert à rien) et les features lissées par rolling (vol 20, z-score volume) produisent des FAUX POSITIFS massifs (PSI > 10 mesuré sur marché homogène) → features quasi-iid uniquement, vérifié par test (plat → MODERATE max, vol x5 → SEVERE). DÉMO == RÉAL (aucun flag de mode). Preuve live : `drift_psi` exposé, decay 0.98 appliqué, n_bars=120 → aucun signal fabriqué sur échantillon insuffisant | commit LOT D · `test_lot_d_drift.py` (26 tests) |
